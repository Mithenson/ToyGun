...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMFeedbackParticles : MMFeedback
    {
        ...
        [MMFEnumCondition("Mode", (int)Modes.Stop)]
        ...
        protected virtual void PlayParticles(Vector3 position)
        {
            ...
            if (RandomParticleSystems.Count > 0)
            {
                ...
                {
                    case Modes.Play:
                        RandomParticleSystems[random].Play(WithChildren);
                        break;
                    case Modes.Stop:
                        RandomParticleSystems[random].Stop(WithChildren, StopBehavior);
                        break;
                    case Modes.Pause:
                        RandomParticleSystems[random].Pause(WithChildren);
                        break;
                }
                return;
            }
            else if (BoundParticleSystem != null)
            {
                ...
                {
                    case Modes.Play:
                        BoundParticleSystem?.Play(WithChildren);
                        break;
                    case Modes.Stop:
                        BoundParticleSystem?.Stop(WithChildren, StopBehavior);
                        break;
                    case Modes.Pause:
                        BoundParticleSystem?.Pause(WithChildren);
                        break;
                }
            }
        }
