...
namespace Ara{
    ...
    public class AraTrail : MonoBehaviour {
        ...
        private UnityAction<CinemachineBrain> cinemachineRenderCallback;
        ...
        private void AttachToCameraRendering()
        {
            ...
            /*cinemachineRenderCallback = brain => { UpdateTrailMesh(brain.OutputCamera); };
            CinemachineCore.CameraUpdatedEvent.AddListener(cinemachineRenderCallback);*/
#endif
...
#if (UNITY_2019_1_OR_NEWER)
            RenderPipelineManager.beginCameraRendering -= renderCallback;
            //CinemachineCore.CameraUpdatedEvent.RemoveListener(cinemachineRenderCallback);
#endif
            Camera.onPreCull -= UpdateTrailMesh;
        }
        ...
        private void PhysicsStep(float timestep)
        {
            ...
            for (int i = 0; i < points.Count; ++i)
            {
                ...
                Debug.Log($"[{Time.frameCount}/{Time.fixedTime}][{i}] Velocity: (X={point.velocity.x}, Y={point.velocity.y}), Magnitude: {point.velocity.magnitude}, Step: {timestep}");
        ...
        private void UpdatePointsLifecycle()
        {
            ...
            for (int i = points.Count - 1; i >= 0; --i)
            {
                ...
                if (point.life <= 0)
                {
                    ...
                    if (smoothness <= 1)
                    {
                        Debug.Log($"REMOVAL [{Time.frameCount}/{Time.fixedTime}][{i}] Position: (X={point.position.x}, Y={point.position.y}), Life: {point.life}");
                        points.RemoveAt(i);
                    }
